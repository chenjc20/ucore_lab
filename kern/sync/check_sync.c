...
//--------into routine in monitor--------------
     ...
     // try to get fork
      // I am hungry
      state_condvar[i]=HUNGRY; //set cond var as hungry
      // try to get fork
      phi_test_condvar(i); 
      if (state_condvar[i] != EATING) //did not get forks,then wait
      {
          cprintf("phi_take_forks_condvar: %d didn't get fork and will wait\n",i);
          cond_wait(&mtp->cv[i]);
      }
//--------leave routine in monitor--------------
      if(mtp->next_count>0)//if exist waiting phi, then wake him up
         up(&(mtp->next));
      else
         up(&(mtp->mutex));
}
...
//--------into routine in monitor--------------
     ...
     // test left and right neighbors
      // I ate over 
      state_condvar[i]=THINKING; // set cond var as thinking
      // test left and right neighbors
      phi_test_condvar(LEFT);
      phi_test_condvar(RIGHT);
//--------leave routine in monitor--------------
     if(mtp->next_count>0)
        up(&(mtp->next));//if exist waiting phi, then wake him up
     else
        up(&(mtp->mutex));
}
