...
sfs_io_nolock(struct sfs_fs *sfs, struct sfs_inode *sin, void *buf, off_t offset, size_t *alenp, bool write) {
  ...
  //LAB8:EXERCISE1 2020210929 chenjunchao HINT: call sfs_bmap_load_nolock, sfs_rbuf, sfs_rblock,etc. read different kind of blocks in file
	/*
	...
	*/
    ...
    if (blkoff != 0) //offset is aligned
    {
        size = (nblks != 0) ? (SFS_BLKSIZE - blkoff) : (endpos - offset);// get the r/w size
        ret = sfs_bmap_load_nolock(sfs, sin, blkno, &ino);//get the ino of the disk block
        if (ret != 0) goto out;
        ret = sfs_buf_op(sfs, buf, size, ino, blkoff);// 将数据wirte to disk or read from disk
        if (ret != 0) goto out;
    ...
    size = SFS_BLKSIZE;
    while (nblks != 0) //读取中间部分的数据，将其分为size大学的块，然后一次读一块直至读完
    {
        ret = sfs_bmap_load_nolock(sfs, sin, blkno, &ino);// 如果存在，首先获取这些数据块对应到磁盘上的数据块的编号
        if (ret != 0)  goto out;
        ret = sfs_block_op(sfs, buf, ino, 1);// 将磁盘上的这些数据块进行读或写操作
        if (ret != 0)  goto out;
        alen += size, buf += size, blkno ++, nblks --;
    }

    if ((size = endpos % SFS_BLKSIZE) != 0) //读取最后一部分的数据
    {
        ret = sfs_bmap_load_nolock(sfs, sin, blkno, &ino);
        if (ret != 0)  goto out;
        ret = sfs_buf_op(sfs, buf, size, ino, 0);
        if (ret != 0)  goto out;// 进行非整块的读或者写操作
        alen += size;
    }
